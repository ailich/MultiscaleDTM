...
SurfaceArea_optimized<- function(r, na.rm=FALSE, filename=NULL, overwrite=FALSE, wopt=list()){
  og_class<- class(r)[1]
  ...
  SA<- terra::focalCpp(r, w=c(3,3), fun = C_SurfaceArea_optimized,  x_res = terra::res(r)[1], y_res = terra::res(r)[2], na_rm=na.rm, fillvalue=NA, wopt=wopt)
  names(SA)<- "SA"
  #Return
  if(og_class =="RasterLayer"){
    SA<- raster::raster(SA)
