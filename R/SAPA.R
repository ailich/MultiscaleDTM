...
#' @param slope_layer Optionally specify an appropriate slope layer IN RADIANS to use. If not supplied, it will be calculated using the SlopeAspect function based on Misiuk et al (2021). The slope layer should have a window size that is 2 larger than the w specified for SAPA.
  ...
  if(is.null(slope_layer)){
    if (all(w==1)){
      slope_layer<- terrain(r, opt="slope", unit="radians", neighbors=8)
        ...
        slope_layer<- SlopeAspect(r, w=w+2, unit="radians", method="queen", metrics= "slope", include_scale=FALSE)
      }
    }
