...
#' @param r DTM as a SpatRaster, RasterLayer, RasterStack, or RasterBrick
...
#' crs = "EPSG:27200")
#' rdmv<- RDMV_Old(r, w=c(5,5), na.rm = TRUE, method="range")
#' plot(rdmv)
#' @import terra
...
RDMV_Old<- function(r, w=c(3,3), method="range", na.rm=FALSE, include_scale=FALSE, filename=NULL, overwrite=FALSE, wopt=list()){
  og_class<- class(r)[1]
