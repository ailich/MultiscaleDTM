...
VRM_optimized<- function(r, w=c(3,3), na.rm = FALSE, include_scale=FALSE, filename=NULL, overwrite=FALSE, wopt=list()){
  og_class<- class(r)[1]
  ...
  scale.factor <- if (!na.rm) {
    ...
    terra::focalCpp(Zrast, w = w, fun = C_CountVals_optimized, wopt = wopt)
  }
