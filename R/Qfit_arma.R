...
Qfit_arma<- function(r, w=c(3,3), unit= "degrees", metrics= c("elev", "qslope", "qaspect", "qeastness", "qnorthness", "profc", "planc", "twistc", "meanc", "maxc", "minc", "features"), slope_tolerance=1, curvature_tolerance=0.0001, outlier_quantile=c(0.01, 0.99), na.rm=FALSE, force_center=FALSE, include_scale=FALSE, mask_aspect=TRUE, return_params= FALSE, as_derivs= FALSE, filename=NULL, overwrite=FALSE, wopt=list()){
  ...
  if((!na.rm) & (!force_center)){
    params<- terra::focalCpp(r, w=w, fun = C_Qfit1_narmF_arma, X= X, Xt= Xt, XtX_inv= XtX_inv, fillvalue=NA, wopt=wopt)
    } else if(na.rm & (!force_center)){
      params<- terra::focalCpp(r, w=w, fun = C_Qfit1_narmT_arma, X_full= X, fillvalue=NA, wopt=wopt)
    } else if((!na.rm) & force_center){
      params<- terra::focalCpp(r, w=w, fun = C_Qfit2_narmF_arma, X= X, Xt= Xt, XtX_inv= XtX_inv, fillvalue=NA, wopt=wopt)} else{
        params<- terra::focalCpp(r, w=w, fun = C_Qfit2_narmT_arma, X_full= X, fillvalue=NA, wopt=wopt)
      }
