...
SAPA_TEST<- function(r=NULL, w = 1, slope_correction=TRUE, na.rm=FALSE, include_scale=FALSE, slope_layer= NULL, sa_layer = NULL, filename=NULL, overwrite=FALSE, wopt=list()){
  ...
  if(is.null(slope_layer) & slope_correction){
    if (all(w==1) & (!na.rm)){
      slope_layer<- terra::terrain(r, v="slope", unit="radians", neighbors=8, wopt=wopt)
        ...
        slope_layer<- SlpAsp_TEST(r, w=w+2, unit="radians", method="boundary", metrics= "slope", na.rm=na.rm, include_scale=FALSE, wopt=wopt)
      }
  }
