...
for (i in 1:nlyr(qmetrics)) {
  ...
  if(!grepl(pattern = "^features", curr_var)){
    ...
    tmap_save(qfit_plt, filename = paste0(R_fig_dir, "qfit/qfit", i_txt, ".png"), dpi=96, width =3, height = 2, units = "in")
    } else{
      png(filename = paste0(R_fig_dir, "qfit/qfit", i_txt, ".png"), res = 96, width =3.5, height = 3, units = "in")
      qfit_plt<- plot(qmetrics[[i]],col=curr_pal, axes=FALSE, box=FALSE, cex=0.1)
      dev.off()
      } 
}

qfit_plt_files<- list.files(paste0(R_fig_dir, "qfit"), pattern = "qfit\\d{2}.png$", full.names = TRUE)
