...
#' @param radius Vector of length 2 specifying c(inner, outer) radii of the annulus c(inner,outer) in "cell" or "map" units. Inner radius must be less than or equal to outer radius.
...
#' @param unit Unit for radius. If the annulus window is specified with the radius parameter either "cell" (number of cells) or "map" for map units (e.g. meters). Otherwise it should be NA or NULL. If radius is specified the default is "cell", otherwise if w is specified, the default is NA.
#' @param na.rm Logical indicating whether or not to remove NA values before calculations.
#' @param w A focal weights matrix created MultiscaleDTM::annulus_window or w can be set to NA or NULL and radius can be used instead.
...
BPI<- function(r, radius, stand="none", 
               unit=ifelse((!is.matrix(w)) & all(is.null(w) | is.na(w)), "cell", NA_character_), 
               w=NULL, na.rm=FALSE, include_scale =FALSE, 
  ...
  bpi<- MultiscaleDTM::RelPos(r, w=w, shape= "annulus", radius=radius, stand=stand, 
