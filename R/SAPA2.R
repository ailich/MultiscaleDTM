...
SAPA2<- function(r, w = 1, slope_correction=TRUE, include_scale=FALSE, slope_layer= NULL){
  og_class<- class(r)[1]
  ...
  SA<- SurfaceArea2(r, expand=FALSE)
  ...
  if(is.null(slope_layer)){
    if (all(w==1)){
      slope_layer<- terra::terrain(r, v="slope", unit="radians", neighbors=8)
      } else{
        slope_layer<- SlpAsp2(r, w=w+2, unit="radians", method="queen", metrics= "slope", include_scale=FALSE)
      }
    }
