...
#' @param mask_aspect A logical. If all values in the window are the same aspect (and therefore northness/eastness) will be set to NA when mask_aspect is TRUE (the default). This is more computationally expensive however. If FALSE aspect will to 270 degrees or 3*pi/2 radians (atan2(0,0)-pi/2+2*pi)
...
WoodEvans2<- function(r, w=c(3,3), unit= "degrees", return_aspect= FALSE, slope_tolerance=1, curvature_tolerance=0.01, na.rm=FALSE, pad=FALSE, include_scale=FALSE, mask_aspect=TRUE, return_params= FALSE, run_in_blocks){
